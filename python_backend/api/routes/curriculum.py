...
async def get_all_plans(
    ...
    """جلب كل اللوائح الدراسية — requires any authenticated user"""
    try:
...
async def get_plan_full(
    ...
    """جلب لائحة كاملة بكل تفاصيلها — requires any authenticated user"""
    try:
        ...
        elective_groups = (
            supabase.table("elective_groups")
            .select("*")
            .eq("plan_id", plan_id)
            .execute()
        )
        for group in elective_groups.data:
            group_courses = (
                supabase.table("elective_group_courses")
        ...
        load_rules = (
            supabase.table("academic_load_rules")
            .select("*")
            .eq("plan_id", plan_id)
            .execute()
        )
        grad_requirements = (
            supabase.table("graduation_requirements")
            .select("*")
            .eq("plan_id", plan_id)
            .execute()
        )
...
async def get_plan_courses(
    ...
    """جلب مواد لائحة معينة — requires any authenticated user"""
    try:
        ...
        return {"courses": result.data, "count": len(result.data)}
    except Exception as e:
...
async def get_course_prerequisites(
    ...
    """جلب المتطلبات السابقة — requires any authenticated user"""
    try:
        result = (
            supabase.table("prerequisites")
        ...
        return {"course_id": course_id, "prerequisites": result.data}
    except Exception as e:
...
async def get_elective_groups(
    ...
    """جلب المجموعات الاختيارية — requires any authenticated user"""
    try:
        ...
        return {"elective_groups": result.data}
    except Exception as e:
...
async def get_grading_scales(
    ...
    """جلب جداول التقديرات — requires admin | academic_advisor"""
    try:
        ...
        for scale in scales.data:
            items = (
                supabase.table("grade_scale_items")
                .select("*")
